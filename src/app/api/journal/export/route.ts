...
export async function GET(request: Request) {
  ...
  if (format === 'pdf') {
    const html = `<html lang="fr"><head><meta charset="utf-8"><title>Mar-ci Flow - Journal</title><style>body{font-family:Arial;color:#172033;padding:32px}h1{color:#2251e5}table{border-collapse:collapse;width:100%}th,td{border:1px solid #dce2eb;padding:8px;text-align:left;vertical-align:top}th{background:#f1f4f9}</style></head><body><h1>MAR-CI FLOW</h1><h2>Journal de travail - ${escapeHtml(auth.ctx.user.firstName)} ${escapeHtml(auth.ctx.user.lastName)}</h2><p>${escapeHtml(roleLabel(auth.ctx.orgRole ?? 'EMPLOYEE'))} · ${escapeHtml(auth.ctx.user.department?.name ?? '')} · ${escapeHtml(auth.ctx.organization.name)}</p><table><thead><tr><th>Date</th><th>Titre</th><th>Catégorie</th><th>Résumé</th><th>Temps</th><th>Statut</th></tr></thead><tbody>${rows.map((row) => `<tr>${row.map((cell) => `<td>${escapeHtml(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table><script>window.print()</script></body></html>`;
    return new NextResponse(html, { headers: { 'Content-Type': 'text/html; charset=utf-8', 'Content-Disposition': 'inline; filename="mar-ci-flow-journal.html"' } });
  }
  const csv = [['Date', 'Titre', 'Catégorie', 'Résumé', 'Temps', 'Statut'], ...rows].map((row) => row.map((cell) => `"${String(cell).replaceAll('"', '""')}"`).join(';')).join('\r\n');
  return new NextResponse(`\ufeff${format === 'excel' ? `<table><tr>${['Date', 'Titre', 'Catégorie', 'Résumé', 'Temps', 'Statut'].map((header) => `<th>${escapeHtml(header)}</th>`).join('')}</tr>${rows.map((row) => `<tr>${row.map((cell) => `<td>${escapeHtml(cell)}</td>`).join('')}</tr>`).join('')}</table>` : csv}`, { headers: { 'Content-Type': format === 'excel' ? 'application/vnd.ms-excel; charset=utf-8' : 'text/csv; charset=utf-8', 'Content-Disposition': `attachment; filename="mar-ci-flow-journal.${format === 'excel' ? 'xls' : 'csv'}"` } });
}