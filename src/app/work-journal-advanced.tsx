...
function EntryForm({ entry, onClose, onSave }: FormProps) {
  return <div className="modal-backdrop" onClick={onClose}><form className="modal panel journal-form" onSubmit={onSave} onClick={(event) => event.stopPropagation()}><button className="modal-close" type="button" onClick={onClose}>×</button><p className="eyebrow">JOURNAL DE TRAVAIL</p><h2>{entry.id ? 'Modifier une entrée' : 'Nouvelle entrée'}</h2><div className="two-col"><label>Date<input name="entryDate" type="date" required defaultValue={entry.entryDate.slice(0, 10)} /></label><label>Catégorie<select name="category" defaultValue={entry.category.name}>{categories.map((item) => <option key={item}>{item}</option>)}</select></label></div><label>Travail réalisé<input name="title" required defaultValue={entry.title} /></label><label>Résumé<textarea name="summary" required defaultValue={entry.summary} /></label><label>Travaux réalisés<textarea name="workItems" placeholder="Un élément par ligne" defaultValue={entry.workItems} /></label><div className="two-col"><label>Temps (minutes)<input name="timeMinutes" type="number" min="0" defaultValue={entry.timeMinutes} /></label><label>Statut<select name="status" defaultValue={entry.status}>{Object.entries(statuses).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label></div><label>Difficultés<textarea name="difficulties" /></label><label>Solutions<textarea name="solutions" /></label><label>Compétences acquises<textarea name="skills" /></label><label>Résultat<textarea name="result" defaultValue={entry.result ?? ''} /></label><button className="primary-button" type="submit">Enregistrer</button></form></div>;
}
